# Hi 👋, I'm Samir Majhi

🎓 Engineering Student | 💻 Tech Enthusiast | 📊 Research-Oriented Learner  

I focus on **engineering analysis**, **software-based modeling**, and applying  
**technology to solve real-world infrastructure problems**.

---

## 🔧 Skills & Tools

- **Programming:** Python, OOP  
- **Engineering Software:** EPANET, AutoCAD, Civil 3D, Diana 
- **Analysis & Research:** Modeling, Risk Assessment, Technical Writing  
- **Version Control:** Git & GitHub  

---

## 📊 GitHub Analytics

### 📈 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=samir-m2jhi&show_icons=true)

### 🔥 GitHub Streak
![GitHub Streak](https://streak-stats.demolab.com?user=samir-m2jhi)

### 🧠 Most Used Languages
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=samir-m2jhi&layout=compact)

---

## 📉 Contribution Activity
![Contribution Graph](https://github-readme-activity-graph.vercel.app/graph?username=samir-m2jhi)

---

## 📌 Featured Projects

🔹 **Water Leakage Detection using EPANET**  
Pressure-based modeling for identifying leakage locations in water networks.

🔹 **Risk Assessment of Buildings in Dense Urban Areas**  
Impact analysis of illegal floor additions and adjacent construction activities.

🔹 **Python Scripts for Engineering Applications**  
Automation and calculation tools for academic and practical use.

---

## 🌱 Currently Learning

- Advanced Python for Engineering Analysis  
- GitHub Automation & APIs  
- Research Methodology and Technical Writing  

---

## 🔗 Connect With Me

- GitHub: https://github.com/samir-m2jhi 

---

⭐ *Thanks for visiting my GitHub profile!*
